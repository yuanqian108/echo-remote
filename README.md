...
- ⏩ 倍速切换(0.5x ~ 3x)
- 📱 移动端自适应,支持系统媒体会话(安卓 Chrome / iOS 锁屏控制)
...
- **v1.4.1**
  - 回滚命令接口"直接返回状态"的行为(播放器命令异步生效,返回的是旧状态,导致界面先旧后新产生闪烁与延迟感);
  - 桥接程序回滚为单线程单请求模式(多线程 keep-alive 在部分浏览器连接复用场景下反而带来间歇延迟);
  - 主题色改为切歌时更新一次,不再每 500ms 读取主题过渡动画的中间色(颜色不再乱跳);
  - 进度条/音量条恢复细样式(6px/4px,拇指 16px)。


- **v1.4.0**
  - 响应速度全面优化:状态接口改为渲染进程内直读(不再每次 IPC 序列化整份歌词,亚毫秒级);
  - 命令接口直接返回最新状态,发命令与拿结果合并为一次往返(切歌/跳转/播放暂停即时生效);
  - 桥接程序重写:每连接独立线程 + HTTP/1.1 keep-alive(免去每次轮询的 TCP 握手),空闲 5 秒自动断开;
  - 轮询间隔 1s → 500ms,命令后的快速回刷精简为 3×250ms。


- **v1.3.0**
  - 控制手感优化:播放/暂停图标按下即翻转(乐观更新),任何命令后 150ms 快速回刷约 1.2 秒,切歌与跳转几乎即时显示;
  - 按钮 120ms 防误触 + Android 微震动反馈(进度条/音量松手也有);
  - 进度条、音量拖动时数值高亮为主题色,滑杆加大触控区并禁止页面滚动干扰;
  - 封面切换淡入过渡;
  - 修正倍速显示(1.5x 此前可能被显示成 1x)。


- **v1.2.0**
  - 移除访问令牌功能(局域网明文 HTTP 下防护意义有限,徒增使用门槛);
  - 设置面板重新设计:状态徽章、访问地址卡片、分组配置,并优先使用宿主原生控件;
  - 「打开页面」按钮移除(宿主窗口打开行为不可控),保留「复制地址」。


- **v1.1.0**
  - 桥接程序启动后主动上报本机局域网地址,设置面板立即显示访问地址(不再依赖反向轮询);
  - 设置面板状态显示优化:未获取地址时显示「运行中」而非「正在获取局域网地址」;
  - 「复制地址」按钮无地址时给出提示,不再静默失效;「打开页面」按钮已移除;
  - 音量调节拖动时实时生效(80ms 节流),不再等松手;
  - 进度条本地时钟驱动,消除网络延迟导致的进度回退;
  - 兼容旧版桥接 /__info 仅返回 ip 单字段的情况。

## 发布到官方插件源

1. 在 GitHub 新建公开仓库(如 `echo-remote`),把本目录**全部文件**(含 `bin/` 下的桥接程序和根目录的 `echo-plugins.json`)上传到仓库根;
2. Fork [EchoMusicPlugins](https://github.com/hoowhoami/EchoMusicPlugins),在 fork 的 `echo-plugins.json` 的 `plugins` 数组末尾追加一条记录:

```json
{
  "id": "echo-remote",
  "path": "",
  "repo": "https://github.com/<你的用户名>/echo-remote",
  "homepage": "https://github.com/<你的用户名>/echo-remote",
  "tags": ["remote", "lan", "web", "control"]
}
```

3. 提交 Pull Request,合并后所有用户就能在 EchoMusic 插件管理的官方插件源里看到并安装。

> 合并前,朋友也可以直接使用你的仓库:插件管理 → 添加插件源 → 输入 `https://github.com/<你的用户名>/echo-remote`。

> 注意:桥接程序必须直接提交进仓库(EchoMusic 安装时下载的是仓库 zip,二进制不在仓库里就无法启动);不要使用 Git LFS。更新插件只需改 `manifest.json` 的版本号并推送,用户端会自动提示更新。